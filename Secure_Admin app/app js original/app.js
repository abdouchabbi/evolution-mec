document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleNavigation(e) {
        ...
        if (sectionId === 'settings-section') {
            // Placeholder for future settings logic if any
        }
    }
    ...
    async function handleAddUser(e) {
        ...
        try {
            await api.post('users/register', { name, email, password, username: email });
    ...
    document.getElementById('show-qr-btn').addEventListener('click', () => {
        ...
        if (!kioskId) {
            return showAlert('Kiosk ID non trovato. Per favore, effettua nuovamente il login.');
        }

        const qrContainerHtml = `
            <h3 class="text-lg font-bold mb-4 text-center">Scansiona per configurare il Kiosk</h3>
            <div id="qrcode-container" class="flex justify-center p-4 bg-white"></div>
            <div class="mt-4 flex justify-end">
                <button id="modal-ok-btn" class="bg-blue-600 text-white px-4 py-2 rounded-md">Chiudi</button>
            </div>`;
        
        alertModalContent.innerHTML = qrContainerHtml;
        document.getElementById('modal-ok-btn').onclick = hideAlert;
        alertModal.classList.remove('hidden');
        alertModal.classList.add('flex');
        setTimeout(() => alertModalContent.classList.remove('scale-95', 'opacity-0'), 10);
    ...
    function startup() {
        ...
        // Holiday/Leave Listeners - These elements are not in the provided admin.html
        // To prevent errors, we check if the element exists before adding a listener
        ...
        if(holidaysTableBody) holidaysTableBody.addEventListener('click', handleDeleteHoliday);
        
        const addLeaveBtn = document.getElementById('add-leave-btn');
        if (addLeaveBtn) addLeaveBtn.addEventListener('click', handleAddLeave);

        const leaveTableBody = document.getElementById('leave-table-body');
        if (leaveTableBody) leaveTableBody.addEventListener('click', handleDeleteLeave);
